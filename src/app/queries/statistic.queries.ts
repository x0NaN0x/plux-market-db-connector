// Gets average price, last purchase date and total amount sold for an item
...
const MOST_EXPENSIVE_ITEM_SOLD_THIS_WEEK = `
SELECT vl.*, idb.name_english, idb.type
  FROM vending_log vl 
  LEFT JOIN item_db AS idb ON vl.item_id = idb.id 
...
export const statisticQueriesTokens = Object.keys(statisticQueries).map(
  (key) => key
);
